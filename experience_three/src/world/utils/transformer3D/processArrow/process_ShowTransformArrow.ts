...
export function process_ShowTransformArrow(tfs: Transformer3D) {
  ...
  (<THREE.Mesh>tfs.stuff.m_Object3D).geometry.computeBoundingBox()
  const vec3 = new THREE.Vector3();
 (<THREE.Mesh>tfs.stuff.m_Object3D).geometry.boundingBox.getCenter(vec3)
  tfs.controller_3d.children[0].position.set(vec3.x,vec3.y,vec3.z);
